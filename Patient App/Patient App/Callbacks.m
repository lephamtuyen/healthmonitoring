...
- (void)handleConnectSuccess
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        [messenger subscribe:[TopicFactory getEventTopic:IOTRecomendEvent] qos:0];
    });
}
...
- (void)routeMessage:(NSString *)topic payload:(NSString *)payload
{
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if ([topicParts[1] isEqualToString:IOTRecomendEvent])
        {
            [self processRecommendationMessage:payload];
        }
    });
}
...
- (void)processRecommendationMessage:(NSString *)payload
{
    AppDelegate *appDelegate = [[UIApplication sharedApplication] delegate];
    NSDictionary *body;
    ...
    NSString *textValue = [body objectForKey:JSON_TEXT];
    if (textValue)
    {
        ...
        Messenger *messenger = [Messenger sharedMessenger];
        [messenger.logMessages addObject:msg];
